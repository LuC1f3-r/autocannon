...
async function generateCharts(result) {
  ...
  if (result.requestsTimeline && Object.keys(result.requestsTimeline).length > 0) {
    const timeKeys = Object.keys(result.requestsTimeline)
  ...
  if (result.statusCodeStats && Object.keys(result.statusCodeStats).length > 0) {
    statusCodesChart = {
      type: 'pie',
      data: {
  ...
  // Create chart images
  const latencyImageBuffer = await chartJSNodeCanvas.renderToBuffer(latencyChart);
  const requestsImageBuffer = await chartJSNodeCanvas.renderToBuffer(requestsChart);
  ...
  if (statusCodesChart) {
    charts.statusCodes = await chartJSNodeCanvas.renderToBuffer(statusCodesChart);
  }
...
async function generatePDF(result, config, outputPath) {
  ...
  return new Promise((resolve) => {
    ...
    if (config.payload) {
      ...
      doc.text(JSON.stringify(config.payload, null, 2));
    }
    doc.moveDown(2);
    ...
    // Add status codes chart if available
...
async function runTest(config, name) {
  ...
  Object.assign(testConfig, {
    ...
    latencyParams: {            // More detailed latency params
      p1: true,
      p50: true,
      p75: true,
      p90: true,
  ...
  // Enable requests per second tracking at each second
  testConfig.setupClient = (client) => {
    client.on('response', (client, statusCode, resBytes, responseTime) => {
      const currentSecond = Math.floor(client.counter.totalTime / 1000) + 1;
      if (!client.requestsPerSecond) {
        client.requestsPerSecond = {};
      }
      if (!client.requestsPerSecond[currentSecond]) {
        client.requestsPerSecond[currentSecond] = 0;
      }
      client.requestsPerSecond[currentSecond]++;
    });
  };
  ...
  if (instance.requestsPerSecond) {
    result.requestsTimeline = instance.requestsPerSecond;
  }
...
async function saveTestResults(result, name, outputDir) {
  const resultsPath = path.join(outputDir, `${name.replace(/[^a-z0-9]/gi, "-")}-${Date.now()}.json`);
...
async function generateComparisonReport(testResults, outputDir) {
  ...
  testResults.sort((a, b) => a.result.latency.average - b.result.latency.average);
  ...
  const generateColorGradient = (count) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (ratio < 0.5) {
        ...
        colors.push(`rgba(255, ${Math.round(greenValue)}, ${Math.round(greenValue * 0.5)}, 0.6)`);
      }
    }
    return colors;
  };
  ...
  // Generate requests comparison chart - sort by highest throughput 
  // We need to sort a copy to keep the original order for the rest of the report
  const sortedByThroughput = [...testResults].sort((a, b) => 
    b.result.requests.average - a.result.requests.average);
  ...
  // Create chart images
  const latencyCompBuffer = await chartJSNodeCanvas.renderToBuffer(latencyCompChart);
  const reqCompBuffer = await chartJSNodeCanvas.renderToBuffer(reqCompChart);
  
  // Generate PDF
  const doc = new PDFDocument({ margin: 50 });
  const reportPath = path.join(outputDir, `comparison-report-${Date.now()}.pdf`);
  ...
  doc.moveDown();
  doc.fontSize(12).text(`${testResults.length} scenarios compared`, { align: "center" });
  doc.moveDown(2);
  ...
  doc.moveDown();
  doc.fontSize(12);
  doc.text(`Best Latency: ${testResults[0].name} (${testResults[0].result.latency.average.toFixed(2)} ms)`);
  doc.text(`Best Throughput: ${sortedByThroughput[0].name} (${sortedByThroughput[0].result.requests.average.toFixed(2)} req/sec)`);
  doc.moveDown(2);
  ...
  testResults.forEach((test, index) => {
    doc.text(test.name, startX + colWidth * (index + 1), tableTop, { width: colWidth, align: 'center' });
  });
  
  // Draw horizontal line below header
  doc.moveTo(startX, tableTop + 20)
     .lineTo(startX + colWidth * (testResults.length + 1), tableTop + 20)
     .stroke();
  ...
  const metrics = [
    ...
    { name: 'Avg Latency (ms)', getter: t => t.result.latency.average.toFixed(2) },
    ...
    { name: 'Throughput (KB/s)', getter: t => (t.result.throughput.average / 1024).toFixed(2) }
  ];
  
  metrics.forEach((metric, idx) => {
    // Alternate row colors for better readability
    if (idx % 2 === 0) {
      doc.rect(startX, rowY - 5, colWidth * (testResults.length + 1), 20).fill('#f5f5f5');
      doc.fillColor('black');
    }
    
    doc.text(metric.name, startX, rowY);
    testResults.forEach((test, index) => {
      const value = String(metric.getter(test));
      doc.text(value, startX + colWidth * (index + 1), rowY, { width: colWidth, align: 'center' });
    });
    rowY += 20;
  });
...
if (require.main === module) {
  ...
  app.listen(port, () => {
    ...
    console.log(`Visit http://localhost:${port}/reports to view available reports`);
  });
}
